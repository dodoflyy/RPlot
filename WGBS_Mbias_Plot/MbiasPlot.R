...
parser$add_argument("--Input", "-I", dest="INPUT", help="csv 格式输入数据", required=TRUE)
parser$add_argument("--Output", "-O", dest="OUTPUT", help="输出 pdf 路径", required=TRUE)
...
p1 <- ggplot(plotData) +
  geom_line(aes(Position, Methylated_Percent, color=Context), show.legend = FALSE) +
...
p2 <- ggplot(plotData) +
  geom_line(aes(Position, Total_Count, color=Context)) +
  scale_color_manual(values=colFun) +
